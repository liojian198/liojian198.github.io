...
| 规范名称 | 核心侧重点 | 协议耦合度 | 适合场景 |
| :--- | :--- | :--- | :--- |
| W3C WoT (TD) | Web 化、跨平台、对接 AI 与前端 | 极低（协议无关） | 异构系统集成、AI Agent 操控设备、Web/App 开发 |
| OMA LwM2M | 蜂窝物联网、设备生命周期管理 | 高（绑定 CoAP） | 智能抄表、共享单车、NB-IoT 终端管理 |
| OPC UA | 工业现场、底层设备互联 | 高（自带完整协议栈） | 智能工厂、PLC 互联、工业自动化控制 |
| 云厂商私有物模型 | 商业化云平台快速开发 | 中（绑定特定云服务） | 垂直行业 SaaS 应用、消费级 IoT 硬件 |
| SSN / SOSA Ontology | 科学级语义推理、知识图谱 | 纯本体（不涉及传输协议） | 气象监测、科研数据共享、知识图谱语义检索 |
